...
export interface HqAgent {
  ...
  avatar_emoji: string | null; avatar_url: string | null; accent_color: string | null;
  chat_enabled: boolean; mission_enabled: boolean; created_at: string;
}
...
export async function fetchHqAgents(projectId: string): Promise<HqAgent[]> {
  const { data } = await supabase
    .from("internal_agents")
    .select("id, name, description, avatar_emoji, avatar_url, accent_color, chat_enabled, mission_enabled, created_at")
    .eq("project_id", projectId).order("created_at", { ascending: false });
  return (data ?? []) as HqAgent[];
}

export async function fetchHqRuns(projectId: string, limit = 500): Promise<HqRun[]> {
  const { data } = await supabase
    .from("internal_agent_runs")
    .select(RUN_SELECT)
    .eq("project_id", projectId).order("created_at", { ascending: true }).limit(limit);
  return (data ?? []) as HqRun[];
}

export async function fetchHqMissions(projectId: string): Promise<HqMission[]> {
  const { data } = await supabase
...
export async function fetchHqDeliverables(projectId: string, agentIds: string[], limit = 200): Promise<HqDeliverable[]> {
...
export async function fetchHqApprovals(projectId: string, limit = 500): Promise<HqApproval[]> {
  const { data } = await supabase
    .from("internal_agent_approvals")
    .select("id, agent_id, run_id, tool_name, action_kind, status, reason, requested_at, decided_at")
    .eq("project_id", projectId).order("requested_at", { ascending: false }).limit(limit);
  return (data ?? []) as HqApproval[];
}

const LOOP_LIMIT = 3; // STAGNATION_LIMIT in _shared/agent-loop.ts
