...
import { TrendingUp, TrendingDown, Minus, Info, CheckCircle2, AlertTriangle, AlertOctagon } from "lucide-react";
...
interface TableSpec { columns: string[]; rows: (string | number | null)[][] }
  ...
  return (
    <div className="space-y-8">
      {(report.title || report.summary) && (
        <header>
          {report.title && <h1 className="text-2xl font-semibold tracking-tight">{report.title}</h1>}
          {report.summary && <p className="mt-1 max-w-3xl text-sm text-muted-foreground">{report.summary}</p>}
        </header>
      )}
  ...
  if (chart.type === "pie") {
    ...
    return (
      <ResponsiveContainer width="100%" height={260}>
        <PieChart>
          <Pie data={data} dataKey={valueKey} nameKey={x} cx="50%" cy="50%" outerRadius={90} label>
