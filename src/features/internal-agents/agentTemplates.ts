...
All money-moving actions (refunds, credits, coupons, retries) MUST be proposed for approval — never executed silently.`,
    ...
    max_steps: 12,
    tools: [
      { kind: "db_read", name: "Billing data", description: "Read revenue data.", config: { tables: ["customers", "subscriptions", "invoices"] } },
...
Escalate critical/exploited issues immediately; never run fixes without approval.`,
    ...
    tools: [
      ...
      { kind: "db_read", name: "Dependencies & findings", description: "Read scanned deps + security findings.", config: { tables: ["security_scan_findings"] } },
...
Recommend rotation; never act on secrets without explicit approval.`,
    ...
    max_steps: 10,
    tools: [
      { kind: "db_read", name: "Secret findings", description: "Read detected secrets + config risks.", config: { tables: ["security_scan_findings", "scan_results"] } },
...
Be explicit about assumptions and lead-time risk.`,
    ...
    tools: [
      ...
      { kind: "db_read", name: "Orders/sales", config: { tables: ["product_events"] } },
...
Ask for missing info via a crisp question rather than guessing.`,
    ...
    max_steps: 10,
    tools: [
      { kind: "db_read", name: "Errors & reports", config: { tables: ["error_events", "alerts"] } },
...
Be precise; surface anything that doesn't tie out. Not a substitute for a licensed accountant — flag items for review.`,
    ...
    max_steps: 12,
    tools: [
      { kind: "db_read", name: "Ledger data", config: { tables: ["invoices", "subscriptions", "cost_records"] } },
...
Be conservative; flag covenant/threshold breaches immediately.`,
    ...
    max_steps: 12,
    tools: [
      { kind: "db_read", name: "Cash & costs", config: { tables: ["invoices", "cost_records", "subscriptions", "metrics_snapshots"] } },
      { kind: "edge_function", name: "Alert team", config: { slug: "send-notification" } },
    ],
...
Be objective; document everything. Flag potential fraud signals to a human immediately.`,
    ...
    max_steps: 12,
    tools: [
      { kind: "db_read", name: "Transactions & actions", config: { tables: ["invoices", "admin_actions", "activity_logs"] } },
