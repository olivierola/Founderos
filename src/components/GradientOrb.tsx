...
// GPU shader orb (noise-mixed 3-color glow, YIQ hue rotation, breathing pulse,
// constant rotation) — the agent's premium "presence". Each instance is ONE
// WebGL canvas, so use it only for prominent/few placements; small or repeated
// spots use the lightweight CSS orb (components/AgentOrb). Lazy-loaded so
...
function GradientScene({ config }: { config: Required<GradientOrbConfig> }) {
  ...
  return (
    <mesh geometry={geometry} frustumCulled={false}>
      <shaderMaterial ref={materialRef} vertexShader={vertexShader} fragmentShader={fragmentShader} uniforms={uniforms} transparent depthWrite={false} depthTest={false} />
    </mesh>
  );
}
...
export function GradientOrb({ config: configOverrides, className = "" }: { config?: GradientOrbConfig; className?: string }) {
  ...
  return (
    <div className={`h-full w-full ${className}`} style={transparent ? { background: "transparent" } : { background: config.background }}>
      <Canvas
        gl={{ antialias: true, alpha: transparent, premultipliedAlpha: true }}
        // Force a fully transparent clear so nothing shows a dark square/box
        // around the orb — only the glowing sphere itself is drawn.
        onCreated={({ gl }) => { if (transparent) gl.setClearColor(0x000000, 0); }}
