...
export interface ToolCallsSectionProps {
  ...
  /** Label for the collapsed summary row. Receives the number of calls. */
  ...
  return (
    <div className={cn("w-fit max-w-[35rem]", className)}>
      ...
      {/* Collapsible Content.
          ...
          The open state is deliberately NOT capped by a max-height. A single
          agent run reaches dozens of calls and, once a few payloads are open,
          several thousand pixels — any cap clips the tail of the list with no
          scrollbar to recover it (the page is what scrolls, not this box), so
          the run's last steps become unreachable. The 0fr→1fr grid row
          animates exactly like a max-height but settles on the content's own
          height. */}
      <div
        className={cn(
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    >
                      <p
                        className={cn(
                          "text-xs text-zinc-600 dark:text-zinc-400 font-medium text-left",
                          hasDetails &&
                    ...
                    {hasCategoryText && (
                      <p className="text-[11px] text-zinc-400 dark:text-zinc-500 capitalize text-left">
                    ...
                    {isCallExpanded && hasDetails && (
                      <div className="mt-2 space-y-2 text-[11px] bg-zinc-100 dark:bg-zinc-800/50 rounded-xl p-3 mb-3 w-fit max-w-full">
