// Security source — runs ACTIVE, non-destructive scans that need a real host
// network (TCP port scan, surface enumeration). Passive checks (headers/TLS/
// HTTP) run in the edge; this handles what edge can't.
//
// Consent is enforced server-side: security-scan-poll only ever returns scans
// whose target has a valid, recorded consent. This runner refuses to scan a
// target the platform didn't authorise.
import net from "node:net";
import { rpc, RUNNER_ID, ts } from "../env.js";

// Common service ports we check (non-destructive: connect, then close).
...
// Ports that should essentially never be publicly reachable.
...
async function portScan(host) {
  const open = [];
  // Limited concurrency to stay polite/non-disruptive.
  ...
  if (scan.scan_type === "port_scan" || scan.scan_type === "full") {
    ...
    for (const p of ports) {
      findings.push({
        ...
        detail: p.risky
          ? `${p.service} is reachable on ${host}:${p.port}. Database/admin/cache services should not be publicly exposed — restrict by firewall/VPC.`
...
export async function pollSecurity() {
  ...
  try {
    ...
    // Endpoint may not exist yet on older deployments — stay quiet.
