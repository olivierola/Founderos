-- 0234_plane_workgraph_members.sql
-- Le Workgraph revient à la vie : il ne lit plus `auth.users`.
--
-- Symptôme : la carte restait vide alors que l'espace était plein. Cause : la
-- branche des membres interrogeait `auth.users`, table à laquelle le rôle
-- `authenticated` n'a pas accès. La fonction n'étant pas `security definer`,
-- elle s'exécute avec les droits de l'appelant ; le refus de permission fait
-- échouer la requête ENTIÈRE, et les neuf autres familles disparaissent avec
-- elle. Un seul `join` de trop, et toute la carte s'éteint.
...
create or replace function public.pj_workgraph(
  ...
  select * from (
    ...
    -- Les membres viennent de `workspace_members` et `profiles`, JAMAIS de
    -- `auth.users`.
    --
    -- C'est la correction de fond. `auth.users` n'est pas lisible par le rôle
    -- `authenticated` : une fonction qui la lit sans être `security definer`
    -- se voit refuser l'accès, et le refus fait échouer l'APPEL ENTIER — pas
    -- seulement la branche fautive. D'où un graphe intégralement vide alors
    -- que neuf familles sur dix n'avaient aucun problème.
    --
    -- Passer la fonction en `security definer` aurait « marché » et aurait été
    -- une faute : toutes ses lectures auraient alors contourné la RLS, et
    -- n'importe qui aurait pu lire le graphe d'un autre espace en passant son
    -- identifiant. On change donc la source, pas les droits.
    --
    -- Le départ est `workspace_members` et non `profiles` : c'est
    -- l'appartenance qui définit un membre, et quelqu'un dont la fiche de
    -- profil manque doit quand même apparaître sur la carte.
    union all
    select 'member', wm.user_id,
           coalesce(nullif(pr.full_name, ''), 'Membre'),
           ''::text, null, null, null, 0, null::uuid,
           (
             ...
             where ass.user_id = wm.user_id and p5.dashboard_id = p_dashboard
           )
